...
export async function updateContact(id: number, data: ContactUpdate): Promise<Contact | null> {
  const { rows } = await pool.query<Contact>(
    `UPDATE contacts SET
      ...
      city               = $4,
      price_list_number  = $5,
      delivers_monday    = COALESCE($6, delivers_monday),
      delivers_tuesday   = COALESCE($7, delivers_tuesday),
      delivers_wednesday = COALESCE($8, delivers_wednesday),
      delivers_thursday  = COALESCE($9, delivers_thursday),
      delivers_friday    = COALESCE($10, delivers_friday),
      delivers_saturday  = COALESCE($11, delivers_saturday),
      delivers_sunday    = COALESCE($12, delivers_sunday)
     WHERE id = $1
     RETURNING
      id,
      tango_id AS "tangoId",
...
export async function createContact(data: ContactCreate): Promise<Contact> {
  const { rows } = await pool.query<Contact>(
    `INSERT INTO contacts
      (tango_id, name, phone_normalized, address, city, price_list_number,
